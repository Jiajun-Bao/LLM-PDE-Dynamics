...
def compute_exact_solution_random_ic_vary_Nx(L, k, T, Nx, Nt, r=2.0, spline_obj=None, 
                                             refine_x=20, return_fine=False, use_chebyshev=False):
    """
    ...
    Input:
        ...
        refine_x (int): Spatial refinement factor for accuracy (default: 10)
        return_fine (bool): Whether to return fine grid solution (default: False)
        use_chebyshev (bool): If True, downsample to Chebyshev nodes instead of even grid (default: False)
    Output:
    ...
    ti = np.arange(0, Nt_fine, refine_t)
    if use_chebyshev:
            # Compute Chebyshev nodes (increasing order via leading minus sign)
            i_cheb = np.arange(1, Nx + 1)
            x_cheb = (L/2) * (-np.cos(np.pi * i_cheb / (Nx + 1)))
            u_exact_all = np.zeros((len(ti), len(x_cheb)))
            # Interpolate Spatially
            for i, t_idx in enumerate(ti):
                u_exact_all[i, :] = np.interp(x_cheb, x_fine, u_fine[t_idx, :])
    else:
        # Even grid downsampling
        xi = np.arange(refine_x, Nx_fine-refine_x, refine_x)
